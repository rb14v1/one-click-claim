...
const tealTheme = createTheme({
    palette: {
        primary: {
            main: BRAND_COLORS.primary_teal,
        },
    },
    components: {
        MuiInputBase: {
            styleOverrides: {
                input: {
                    accentColor: BRAND_COLORS.primary_teal,
                    '&::selection': {
                        backgroundColor: 'rgba(33, 176, 190, 0.3)',
                    },
                    '&::-webkit-datetime-edit-day-field:focus': {
                        backgroundColor: BRAND_COLORS.primary_teal,
                        color: 'white',
                        outline: 'none',
                    },
...
export const ReviewPage = () => {
    ...
    // Applies an orange background & border to MUI fields if missing
    const getFieldClass = (val: any, isRequired: boolean = true, extraClasses: string = '') => {
        const baseClass = !isRequired || hasVal(val)
            ? '!bg-white transition-all duration-300'
            : '!bg-orange-50/60 [&_fieldset]:!border-orange-300 [&_label]:!text-orange-800 transition-all duration-300 rounded-md shadow-sm';
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                                        ...
                                        <div className="md:col-span-12 grid grid-cols-1 md:grid-cols-3 gap-6">
                                            <TextField
                                                ...
                                                className={getFieldClass(currentReceipt.data.date)}
                                            />
                                            <div className="md:col-span-2">
                                                <TextField label="Vendor / Merchant" placeholder="Who did you pay?" value={currentReceipt.data.merchant_name || ''} onChange={(e) => handleInputChange('merchant_name', e.target.value)} fullWidth size="small" InputLabelProps={{ shrink: true }} InputProps={{ startAdornment: <InputAdornment position="start"><Storefront fontSize="small" className="text-gray-400" /></InputAdornment> }} className={getFieldClass(currentReceipt.data.merchant_name)} />
                                            </div>
                                        </div>

                                        {/* MOVED UP: Location, Attendees, Description */}
                                        <div className="md:col-span-12">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                {showLocationInputs && (
                                                    <>
                                                        <TextField label="Start Location" value={currentReceipt.data.start_location || ''} onChange={(e) => handleInputChange('start_location', e.target.value)} fullWidth size="small" InputLabelProps={{ shrink: true }} InputProps={{ startAdornment: <InputAdornment position="start"><MapOutlined fontSize="small" className="text-gray-400" /></InputAdornment> }} className={getFieldClass(currentReceipt.data.start_location, showLocationInputs)} />
                                                        <TextField label="End Location" value={currentReceipt.data.end_location || ''} onChange={(e) => handleInputChange('end_location', e.target.value)} fullWidth size="small" InputLabelProps={{ shrink: true }} InputProps={{ startAdornment: <InputAdornment position="start"><MapOutlined fontSize="small" className="text-gray-400" /></InputAdornment> }} className={getFieldClass(currentReceipt.data.end_location, showLocationInputs)} />
                                                    </>
                                                )}
                                                {isAttendeeGroup && (
                                                    <div className="md:col-span-2">
                                                        <TextField label="Attendees (Staff/Clients)" value={currentReceipt.data.attendees || ''} onChange={(e) => handleInputChange('attendees', e.target.value)} fullWidth size="small" InputLabelProps={{ shrink: true }} InputProps={{ startAdornment: <InputAdornment position="start"><PeopleAltOutlined fontSize="small" className="text-gray-400" /></InputAdornment> }} className={getFieldClass(currentReceipt.data.attendees, isAttendeeGroup)} />
                                                    </div>
                                                )}
                                                <div className="md:col-span-2">
                                                    <TextField label="Description / Notes" multiline rows={3} fullWidth value={currentReceipt.data.description || ''} onChange={(e) => handleInputChange('description', e.target.value)} InputLabelProps={{ shrink: true }} placeholder="Enter description..." className="!bg-white" />
                                                </div>
                                            </div>
                                        </div>

                                        {/* MOVED DOWN: Financials & Mileage */}
                                            ...
                                            {isMileageGroup && (
                                                <div className="mb-4">
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                                        <TextField label="Distance (Mileage)" placeholder="e.g. 150" value={currentReceipt.data.mileage || ''} onChange={(e) => handleInputChange('mileage', e.target.value)} fullWidth size="small" InputLabelProps={{ shrink: true }} className={getFieldClass(currentReceipt.data.mileage, isMileageGroup)} />
                                                    </div>
                                                </div>
                                            )}
                                            {showFinancials && (
                                                <div className="mb-2">
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                                                        ...
                                                        <div className="flex gap-2 col-span-1 xl:col-span-2">
                                                            <TextField select label="Currency" value={currentReceipt.data.currency || 'EUR'} onChange={(e) => handleInputChange('currency', e.target.value)} size="small" InputLabelProps={{ shrink: true }} className={getFieldClass(currentReceipt.data.currency, showFinancials, "w-28 shrink-0")}>
                                                                {dynamicCurrencies.map((cur) => <MenuItem key={cur} value={cur}>{cur}</MenuItem>)}
                                                            </TextField>
                                                            <TextField label="Incurred Amount" placeholder="0.00" value={currentReceipt.data.total_amount || ''} onChange={(e) => handleInputChange('total_amount', e.target.value)} fullWidth size="small" InputLabelProps={{ shrink: true }} className={getFieldClass(currentReceipt.data.total_amount, showFinancials)} />
                                                        </div>

                                                        {/* TAX */}
                                                        <div className="col-span-1 xl:col-span-2">
                                                            <TextField select label="Tax Rate" value={currentReceipt.data.tax_rate || ''} onChange={(e) => handleInputChange('tax_rate', e.target.value)} fullWidth size="small" InputLabelProps={{ shrink: true }} className={getFieldClass(currentReceipt.data.tax_rate, showFinancials)}>
                                        ...
                                        {!(isAbsoluteLast && currentReceipt.status === "validated") && (
                                            <Button
                                                ...
                                                className={`
                                                    ...
                                                    ${canValidate
                                                        ? '!bg-[#21b0be] hover:!bg-[#159da9] !text-white'
                                                        : 'disabled:!bg-gray-200 disabled:!text-gray-400'
                                                    }
                                                     `}
                                            >
                                                {currentReceipt.status === "validated" ? "Next" : "Validate & Next"}
                                            </Button>
                                        )}
