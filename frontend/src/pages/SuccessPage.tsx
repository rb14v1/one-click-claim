...
const SuccessPage = () => {
  ...
  // Retrieve data passed from ReviewPage (Added totalAmount and itemCount)
  const { results = [], totalAmount = 0, itemCount = 0 } = location.state || {};
 
  // State to handle transition from Step 4 (Processing) to Step 5 (Processed/Validation)
  const [step, setStep] = useState<4 | 5>(4);
 
  // State to hold and dynamically update the batches
  const [batches, setBatches] = useState<any[]>(results);
  const hasStartedPolling = useRef(false);
 
  useEffect(() => {
    // Only start polling when we move to Step 5
    if (step !== 5) return;
    if (hasStartedPolling.current) return;
    hasStartedPolling.current = true;
 
    // Isolate the polling logic for a SINGLE batch
      ...
      while (!isDone && attempt < maxRetries) {
        ...
        try {
          ...
          if (terminalStates.includes(lowerStatus)) {
            ...
            // Safety net: Stop endless loops if Kantata never updates
            setBatches(prev => {
              const copy = [...prev];
              copy[index] = {
                ...copy[index],
                import_status: 'Timeout',
                import_message: 'Salesforce took too long to process. Check Kantata directly.'
              };
              return copy;
            });
            isDone = true;
        ...
        <div className="w-full max-w-xl animate-in fade-in slide-in-from-bottom-4 duration-500 mt-4">
          <div className="bg-white p-8 md:p-10 rounded-[2rem] shadow-xl border border-gray-100 text-center relative overflow-hidden">
            ...
            <div className="w-20 h-20 bg-teal-50 rounded-full flex items-center justify-center mx-auto mb-6 border-4 border-white shadow-sm ring-1 ring-teal-100">
              ...
              {step === 4 ? (
                <Send className="!text-[#21b0be] !text-4xl ml-1" />
              ) : (
                <CheckCircle className="!text-[#21b0be] !text-5xl" />
              )}
            </div>
            ...
            {/* --- STATS GRID: Only visible in Step 5 --- */}
            {step === 5 && (
              ...
              <div className="space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar pr-1">
                {batches.length > 0 ? (
                  batches.map((batch: any, index: number) => {
                    ...
                    else if (isPolling) badgeColor = 'bg-blue-50 text-blue-700 border-blue-200 animate-pulse';
                    ...
                    return (
                      <div key={index} className="flex flex-col p-4 bg-white border border-gray-200 rounded-xl transition-all duration-200 group">
                        <div className="flex items-center justify-between">
                          <div className="flex flex-col overflow-hidden mr-3">
                            ...
                            <div className="flex items-center gap-2 mt-1 flex-wrap">
                              ...
                              {/* Always show copy button if ID exists */}
                              ...
                              {/* Dynamic Status Badge - ONLY show in step 5 */}
                              {step === 5 && (
                          ...
                          {/* Only show Track button in step 5 */}
                          {step === 5 && (
                            <Button
                              onClick={() => handleOpenLink(batch.target_url)}
                              variant="contained"
                              disabled={!batch.target_url}
                              endIcon={<OpenInNew />}
                        ...
                        {/* Error Message Box (Only shows if Kantata failed it AND we are on step 5) */}
                        {step === 5 && (batch.import_message || batch.error) && (
                          <div className="mt-3 p-3 bg-red-50 border border-red-100 rounded-lg flex items-start gap-2">
                            ...
                            <span className="text-xs text-red-700 font-medium break-words">
                              ...
                              {batch.import_message || batch.error}
                            </span>
                          </div>
                        )}
                      </div>
            ...
            {/* Bottom Button changes based on step */}
            {step === 4 ? (
              <Button
                fullWidth
                onClick={() => setStep(5)}
                variant="contained"
                className="!py-3 !rounded-xl !bg-[#21b0be] hover:!bg-[#159da9] !text-white !font-bold !text-base !normal-case flex items-center gap-2"
              >
              ...
              <Button
                ...
                variant="outlined"
                className="!py-3 !rounded-xl !border-gray-300 !text-gray-600 hover:!bg-gray-50 hover:!border-gray-400 !font-bold !text-base !normal-case flex items-center gap-2"
              >
